...
public class SistemaConstruccion_01 : MonoBehaviour 
{
	[SerializeField]
	...
	[HideInInspector]
	public bool mostrarRampas;
	private bool mostrarParedes;
	[HideInInspector]
	public bool mostrarSuelos;

	public GameObject Paredes;
	public GameObject Suelos;
	public GameObject paredesSuperiores;
	public GameObject Techos;

	public GameObject[] suelosDeshabilitar;

	public GameObject ray;

	public RaycastHit hit;	//Se comunicará con editorScript

	public enum Cardinales { Norte, Sur, Este, Oeste }
	public Cardinales direccion;

	public GameObject ParedesInternas;

	AlturaDinamica alturaDinamica;

	private string collideWith;

	private float yPuntual;

	public GameObject Estructura;
	void Start()
	{
		Visible(visibilidadSuelos, false);
		alturaDinamica = gameObject.GetComponent<AlturaDinamica>();
	}

	public void SacarPlanos(bool valor)
	{
		GameObject.Find("Planos").GetComponent<MeshRenderer>().enabled = valor;
		GameObject.Find("Lapiz").GetComponent<MeshRenderer>().enabled = valor;
	}
	...
	void Update()
	{
		...
		if (puedeConstruir)
		{
			gameObject.GetComponent<Apuntador>().ap = false;
			//gameObject.GetComponent<ControladorTerceraPersona>().Construir(true);
			...
			if (Physics.Raycast(ray.transform.position, ray.transform.forward, out hit, 10f))
			{
				if (hit.collider.tag == "ParedC" || hit.collider.tag == "Pared")
				{
					...
					Vector3 vectorResta = Vector3.Scale(normal, new Vector3(2.5f, 2.5f, 2.5f));					
					Vector3 posOnNormal = hit.transform.position + vectorResta;
					Rampa.transform.position = posOnNormal;
					...
					if (hit.collider.tag == "ParedC")
					{
						...
						if (mostrarParedes)
						{
							if (collideWith != hit.collider.name)
							{
								yPuntual = hit.transform.position.y;
							}

							collideWith = hit.transform.gameObject.name;
							...
							PosicionPared = new Vector3(hit.transform.position.x, yPuntual, hit.transform.position.z);
				...
				}	 //Si choca con pared, la rotación de la rampa se calcula con la normal

				if (mostrarSuelos || mostrarRampas)
				{
					if (hit.collider.tag == "SueloC" || hit.collider.tag == "Suelo")
					{
						//print("Suelo");
						//Si choca con suelo calcula la rotación de las rampas en función de los puntos cardinales
		...
		{
			//gameObject.GetComponent<ControladorTerceraPersona>().Construir(false);
			Techos.GetComponent<BoxCollider>().enabled = false;
	...
	private void MostrarGuias()
	{
		if (mostrarParedes)
		{
			...
			paredesSuperiores.SetActive(true);
			Techos.GetComponent<BoxCollider>().enabled = true;
			for (int i = 0; i < Techos.transform.childCount; i++)
			{
				Transform temp = Techos.transform.GetChild(i);
				temp.GetComponent<BoxCollider>().enabled = true;
			}
			Disponibilidad(suelosDeshabilitar, true);
			alturaDinamica.AlturaOnRampas(false);
		}
		...
		else if (mostrarRampas)
		{
			...
			Paredes.SetActive(true);
			Visible(visibilidadParedes, false);
			paredesSuperiores.SetActive(false);
			Techos.GetComponent<BoxCollider>().enabled = false;
			...
			Disponibilidad(suelosDeshabilitar, false);
			alturaDinamica.AlturaOnRampas(true);
		}
	...
	void Disponibilidad(GameObject[] arreglo, bool disponible)
	{
		for (int i = 0; i < arreglo.Length; i++)
		{
			arreglo[i].SetActive(disponible);
		}
	}
	...
	void PosicionEstructurasGuias()
	{
		...
		{
			AlturaDinamica.yParedesGlobal += 4;
